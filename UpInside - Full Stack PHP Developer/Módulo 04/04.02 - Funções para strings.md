# Funções para strings

## strings e multibyte
Use sempre **multibyte** pois ele interpreta os caracteres acentuados.
```php
$string = "Mangá é bom";

var_dump([
	"string"        => $string,
	"strlen"        => $string,
	"mb_strlen"     => mb_strlen($string),
	"substr"        => substr($string, 5),
	"mb_substr"     => mb_substr($string, 5),
	"strtoupper"    => strtoupper($string),
	"mb_strtoupper" => mb_strtoupper($string),
]);
?>

/* Resultado
Array
(
    [string] => Este é um texto que não tem códito html.
    [mb_strlen] => 40
    [substr] =>  é um texto que não tem códito html.
    [mb_substr] =>  é um texto que não tem códito html.
    [strtoupper] => ESTE é UM TEXTO QUE NãO TEM CóDITO HTML.
    [mb_strtoupper] => ESTE É UM TEXTO QUE NÃO TEM CÓDITO HTML.
)
*/
```

## conversão de caixa
```php
$string = "Este é um texto que não tem códito html.";

var_dump([
	"mb_strtoupper" => mb_strtoupper($string),
	"mb_strtolower" => mb_strtolower($string),
	"mb_convert_case UPPER" => mb_convert_case($string, MB_CASE_UPPER),
	"mb_convert_case LOWER" => mb_convert_case($string, MB_CASE_LOWER),
	"mb_convert_case TITLE" => mb_convert_case($string, MB_CASE_TITLE),
]);

/* Resultado
Array
(
    [mb_strtoupper] => ESTE É UM TEXTO QUE NÃO TEM CÓDITO HTML.
    [mb_strtolower] => este é um texto que não tem códito html.
    [mb_convert_case UPPER] => ESTE É UM TEXTO QUE NÃO TEM CÓDITO HTML.
    [mb_convert_case LOWER] => este é um texto que não tem códito html.
    [mb_convert_case TITLE] => Este É Um Texto Que Não Tem Códito Html.
)
*/
```

## substituição
```php
$string = "Este é um texto que não tem códito html.";

var_dump([
	"mb_strlen" => mb_strlen($string),
	"mb_strpos" => mb_strpos($string, "não"),
	"mb_strrpos" => mb_strrpos($string, "não"),
	"mb_substr" => mb_substr($string, 4),
	"mb_strstr" => mb_strstr($string, "não"),
	"mb_strrchr" => mb_strrchr($string, "não"),
]);

/* Resultado
Array
(
    [mb_strlen] => 40
    [mb_strpos] => 20
    [mb_strrpos] => 20
    [mb_substr] =>  é um texto que não tem códito html.
    [mb_strstr] => não tem códito html.
    [mb_strrchr] => não tem códito html.
)
*/
```

```php
$string = "Este é um texto que não tem códito html.";

echo str_replace('texto', 'documento', $string);

/* Resultado
Este é um documento que não tem códito html.
*/
```

```php
$string = "Este é um texto que não tem códito html.";
$atual = ['texto', 'html'];
$novo = ['documento', 'php'];

echo str_replace($atual, $novo, $string);

/* Resultado
Este é um documento que não tem códito php.
*/
```

## parse string
```php
$endPoint = "name=Thiago&email=meuemail@gmail.com";
mb_parse_str($endPoint, $parseEndPoint);

var_dump([
	$endPoint,
	$parseEndPoint,
	(object) $parseEndPoint,
]);

/* Resultado
array(3) {
  [0] => string(36) "name=Thiago&email=meuemail@gmail.com"
  [1] => array(2) {
    ["name"] => string(6) "Thiago"
    ["email"] => string(18) "meuemail@gmail.com"
  }
  [2]=> object(stdClass)#1 (2) {
    ["name"] => string(6) "Thiago"
    ["email"] => string(18) "meuemail@gmail.com"
  }
}
*/
```

<!--stackedit_data:
eyJoaXN0b3J5IjpbNjk0MTUyMzAsLTEyOTI0NDY5MDZdfQ==
-->